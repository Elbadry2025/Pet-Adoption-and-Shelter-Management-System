...
export interface AdopterRegisterRequest {
    name: string;
    emailAddress: string;
    passwordHash: string;
    phoneNumber: string;
...
const AdopterSignupForm = () => {
  ...
  const handleSignup = async (event: any) => {
    ...
    const request: AdopterRegisterRequest = {name: name, emailAddress: username, passwordHash: password, phoneNumber: phone}
    try {
      const response = await axios(
        `http://localhost:8081/api/auth/adopterRegister`,
        {
            method: 'POST',
            data: request
        }
  ...
  return (
    <section className="vh-100 adopter-login-form">
      ...
      <h1 style={{ position: 'absolute', top: '60px' }}>Adopters Signup</h1>
...
export default AdopterSignupForm;
